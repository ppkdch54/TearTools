...
namespace ZSJCMaster.Models
{
    ...
    public delegate void TcpRecvDelegate(AlarmInfo[] alarms);
    ...
    public class TcpComm : BindableBase
    {
        ...
        /// 各个控制板状态
        /// </summary>
        static Dictionary<int, Statu> ControlPadStatus = new Dictionary<int, Statu>();
        /// <summary>
        /// 获取或者设置网络连接状态
        /// </summary>
                    ...
                    if (bytes[i] == 0x87)
                    {
                        if (bytes[i + 29] == 0x0a)
                        {
                            ...
                            for (int k = 0; k < 5; k++)
                            {
                                ...
                                var data = CopyBytesFromSourceByteArray(bytes, i, i + 29);
                                AlarmInfos[k].OrigionData = ConvertBytesToString(data);
        ...
        private byte[] CopyBytesFromSourceByteArray(byte[] sourceByteArray,int index,int length)
        {
            if (sourceByteArray != null)
            {
                //先将源字节数组复制
                byte[] tempByteArray = new byte[sourceByteArray.Length];
                sourceByteArray.CopyTo(tempByteArray,index);
                byte[] destByteArray = new byte[length];
                for (int i = 0; i < length; i++)
                {
                    destByteArray[i] = tempByteArray[i];
                }
        ...
        public static Statu GetConnectStatu(int controlpadId)
        {
            if(ControlPadStatus.Count <= 0)
            {
                return Statu.Unknown;
            }
            if(ControlPadStatus.ContainsKey(controlpadId))
            {
                return ControlPadStatus[controlpadId];
            }
            return Statu.Unknown;
        }
    }
    ...
    public class AlarmInfo : BindableBase
    {
        ...
        public string OrigionData { get; set; }
